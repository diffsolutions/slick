...
trait JdbcActionComponent extends SqlActionComponent { self: JdbcProfile =>
  ...
  abstract class SimpleJdbcProfileAction[+R](_name: String, val statements: Vector[String])
      ...
      with    ProfileAction[R, NoStream, Effect]
  {
    ...
    final def overrideStatements(_statements: Iterable[String]): ProfileAction[R, NoStream, Effect] = {
      new SimpleJdbcProfileAction[R](_name, _statements.toVector) {
        def run(ctx: Backend # Context, sql: Vector[String]): R =
          self.run(ctx, statements)
      }
    }
  }
  ...
  protected class PushStatementParameters(p: JdbcBackend.StatementParameters)
      extends SynchronousDatabaseAction[Unit, NoStream, Backend, Effect]
  {
    def run(ctx: Backend # Context): Unit =
      ctx.pushStatementParameters(p)

    def getDumpInfo = DumpInfo(name = "PushStatementParameters", mainInfo = p.toString)
  }


  protected object PopStatementParameters
      extends SynchronousDatabaseAction[Unit, NoStream, Backend, Effect]
  {
  ...
  protected class SetTransactionIsolation(ti: Int)
  ...
  class JdbcActionExtensionMethods[E <: Effect, R, S <: NoStream](a: DBIOAction[R, S, E]) {
    ...
    def transactionally: DBIOAction[R, S, E with Effect.Transactional] = {
      SynchronousDatabaseAction
        .fuseUnsafe(
          StartTransaction
            .andThen(a)
            .cleanUp(eo => if (eo.isEmpty) Commit else Rollback)(DBIO.sameThreadExecutionContext)
    ...
    def withTransactionIsolation(ti: TransactionIsolation): DBIOAction[R, S, E] = {
      val isolated =
        (new SetTransactionIsolation(ti.intValue))
          .flatMap(old => a.andFinally(new SetTransactionIsolation(old)))(DBIO.sameThreadExecutionContext)
        ...
        : DBIOAction[R, S, E] =
    {
      new PushStatementParameters(
        JdbcBackend.StatementParameters(rsType,
                                        rsConcurrency,
                                        rsHoldability,
                                        statementInit,
                                        fetchSize))
        .andThen(a)
        .andFinally(PopStatementParameters)
    }
  }
  ...
  def createQueryActionExtensionMethods[R, S <: NoStream](tree: Node, param: Any)
  ...
  def createStreamingQueryActionExtensionMethods[R, T](tree: Node, param: Any)
  ...
  class MutatingResultAction[T](
    ...
    sendEndMarker: Boolean
      ...
      extends SynchronousDatabaseAction[Nothing, Streaming[ResultSetMutator[T]], Backend, Effect]
      with    ProfileAction[Nothing, Streaming[ResultSetMutator[T]], Effect]
  {
    streamingAction =>

    class Mutator(val prit: PositionedResultIterator[T],
                  val bufferNext: Boolean,
                  val inv: QueryInvokerImpl[T])
        extends ResultSetMutator[T]
    {
      val pr = prit.pr
      val rs = pr.rs
      var current: T = _
    ...
    override def emitStream(ctx: Backend # StreamingContext, limit: Long, state: StreamState)
        : StreamState =
    {
      val mu =
    ...
    override def cancelStream(ctx: Backend # StreamingContext, state: StreamState): Unit = {
      state.prit.close()
    }
    ...
    def overrideStatements(_statements: Iterable[String]): MutatingResultAction[T] = {
      new MutatingResultAction[T](rsm,
                                  elemType,
                                  collectionType,
                                  _statements.head,
                                  param,
      ...
      (tree match {
         ...
         case First(rsm @ ResultSetMapping(_, compiled, _)) => {
           ...
           new SimpleJdbcProfileAction[R]("result", Vector(sql)) {
             def run(ctx: Backend # Context, sql: Vector[String]): R = {
               createQueryInvoker[R](rsm, param, sql.head)
                 .first(ctx.session)
             }
           }
         }
      ...
      with    super.StreamingQueryActionExtensionMethodsImpl[R, T]
  {
    override def result: StreamingProfileAction[R, T, Effect.Read] = {
      super
        .result
        .asInstanceOf[StreamingProfileAction[R, T, Effect.Read]]
    }

    /** Same as `mutate(sendEndMarker = false)`. */
    def mutate: ProfileAction[Nothing, Streaming[ResultSetMutator[T]], Effect.Read with Effect.Write] = {
      mutate(false)
    }
    ...
    /** Create an Action that can be streamed in order to modify a mutable result set. All stream
      ...
      * time. The resulting stream is always non-buffered and events can be processed either
    ...
    def mutate(sendEndMarker: Boolean = false)
        : ProfileAction[Nothing, Streaming[ResultSetMutator[T]], Effect.Read with Effect.Write] =
    {
      val sql =
        tree
  ...
  class DeleteActionExtensionMethodsImpl(tree: Node, param: Any) {
    ...
    def delete: ProfileAction[Int, NoStream, Effect.Write] = {
      val ResultSetMapping(_, CompiledStatement(_, sres: SQLBuilder.Result, _), _) =
        tree
      ...
      extends super.SchemaActionExtensionMethodsImpl
  {
      ...
      new SimpleJdbcProfileAction[Unit]("schema.create", schema.createStatements.toVector) {
        ...
        def run(ctx: Backend # Context, sql: Vector[String]): Unit = {
          for (s <- sql) {
            ctx
              .session
              .withPreparedStatement(s)(_.execute)
          }
        }
      }
    }
    ...
    def truncate: ProfileAction[Unit, NoStream, Effect.Schema] = {
      new SimpleJdbcProfileAction[Unit]("schema.truncate" , schema.truncateStatements.toVector) {
        def run(ctx: Backend # Context, sql: Vector[String]): Unit = {
          for (s <- sql) {
            ctx
              .session
  ...
  protected abstract class InsertActionComposerImpl[U](val compiled: CompiledInsert)
      extends InsertActionComposer[U]
  {
    protected[this] def buildQueryBasedInsert[TT, C[_]](query: Query[TT, U, C]): SQLBuilder.Result = {
      compiled
        .forceInsert
        .ibr
        .buildInsert(queryCompiler.run(query.toNode).tree)
    }
        ...
        : SQLBuilder.Result =
    {
      compiled
        .forceInsert
        .ibr
        .buildInsert(compiledQuery.compiledQuery)
    }
    ...
    protected def useBatchUpdates(implicit session: Backend # Session) = {
      session
        .capabilities
    ...
    class MultiInsertAction(a: compiled.Artifacts, values: Iterable[U])
        extends SimpleJdbcProfileAction[MultiInsertResult]("MultiInsertAction", Vector(a.sql))
    {
      def run(ctx: Backend # Context, sql: Vector[String]) = {
        val sql1 = sql.head

        if (!useBatchUpdates(ctx.session) ||
            (values.isInstanceOf[IndexedSeq[_]] && values.asInstanceOf[IndexedSeq[_]].length < 2)) {
          retMany(values,
                  values.map { v =>
    ...
    class InsertOrUpdateAction(value: U)
      ...
      else Vector(compiled.checkInsert.sql, compiled.updateInsert.sql, compiled.standardInsert.sql))
    {
      ...
      protected def nativeUpsert(value: U, sql: String)(implicit session: Backend # Session)
      ...
      protected def emulate(
        value: U, checkSql: String, updateSql: String, insertSql: String
      )(
    ...
    class InsertQueryAction(sbr: SQLBuilder.Result, param: Any)
        extends SimpleJdbcProfileAction[QueryInsertResult]("InsertQueryAction", Vector(sbr.sql))
    {
      def run(ctx: Backend # Context, sql: Vector[String]) = {
  ...
  protected class CountingInsertActionComposerImpl[U](compiled: CompiledInsert)
      extends InsertActionComposerImpl[U](compiled)
      with    CountingInsertActionComposer[U]
  {
    def returning[RT, RU, C[_]](value: Query[RT, RU, C]): ReturningInsertActionComposer[U, RU] = {
    ...
    protected def retMany(values: Iterable[U], individual: Seq[SingleInsertResult]) =
      Some(individual.sum)
  ...
  protected class ReturningInsertActionComposerImpl[U, QR, RU](compiled: CompiledInsert, val keys: Node, val mux: (U, QR) => RU)
      extends InsertActionComposerImpl[U](compiled)
      with    ReturningInsertActionComposer[U, RU]
  {
    ...
    protected def checkInsertOrUpdateKeys: Unit = {
      if (keyReturnOther)
        throw new SlickException("Only a single AutoInc column may be returned from an insertOrUpdate call")
    }
    ...
    protected def retOne(st: Statement, value: U, updateCount: Int) =
      mux(value, buildKeysResult(st).first(null))
    ...
    protected def retOneInsertOrUpdateFromInsert(st: Statement, value: U, updateCount: Int)
        : SingleInsertOrUpdateResult =
    {
