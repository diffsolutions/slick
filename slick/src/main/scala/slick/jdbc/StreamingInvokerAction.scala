    ...
    with    FixedSqlStreamingAction[R, T, E]
{
  ...
  override final def emitStream(ctx: JdbcBackend # StreamingContext, limit: Long, state: StreamState)
      : StreamState =
  {
    val bufferNext = ctx.bufferNext

    // vyrobi iterator, pokud ho nemam v ruce
    // tenhle iterator vytahuje samotny data
    val it =
      if (state ne null)
        state
      else
    ...
    try {
      while (if (bufferNext) { it.hasNext    && count < limit }
        ...
        count += 1
        ctx.emit(it.next()) // = ctx.subscriber.onNext(it.next())
      }
    } catch {
      case NonFatal(ex) =>
    ...
    val stillOpen =
      if (bufferNext) it.hasNext
      else count == limit

    if (stillOpen) it else null
  }

  override final def cancelStream(ctx: JdbcBackend # StreamingContext, state: StreamState): Unit =
    state.close()
  ...
  private[this] class HeadOptionAction(val statements: Iterable[String])
      extends SynchronousDatabaseAction[Option[T], NoStream, JdbcBackend, E]
